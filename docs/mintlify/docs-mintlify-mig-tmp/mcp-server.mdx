...
| `content_type` | string | `all`, `vision`, `audio`, `accessibility`, or `input` (default: `all`) |
...
<Note>
UI events require Accessibility permissions (macOS) or appropriate permissions (Windows) and "UI Events" enabled in screenpipe settings.
</Note>
